...
class LLMClient:
    def __init__(self, config):
        ...
        if not self.api_key:
            raise ValueError(f"未提供 {self.provider} 的API密钥")

        default_models = {
            "openai": "gpt-4",
            "deepseek": "deepseek-chat"
        }
        self.model = config.get("model") or default_models.get(self.provider, "gpt-4")
    ...
    def generate_code(self, prompt):
        """根据用户需求生成模块代码，强制要求包含main函数"""
        messages = [
            {"role": "system", "content": """你是一个Python专家，只生成纯Python代码，不要包含任何额外的文本、解释、示例或说明。代码中只能包含必要的注释（以#开头），不要使用中文注释。代码应该可以直接被Python解释器执行。

生成的模块必须包含一个名为main的函数作为入口。main函数应定义为无参数（def main():）或接受一个列表参数（def main(args=None):），以便在需要时接收用户输入。如果模块需要用户输入，应该在main函数内部使用input()获取，或者通过args参数接收。

如果模块需要第三方库，请在代码开头用注释说明依赖，例如：# requires: requests。可以在注释中提供建议的模块名，例如：# module_name: fib。不要包含任何Markdown标记（如```python）或自然语言描述。"""},
            {"role": "user", "content": prompt}
        ]
        return self._call_chat_completion(messages)

    def fix_code(self, code, error_msg):
        """根据错误信息修复代码，同样只返回纯Python代码"""
